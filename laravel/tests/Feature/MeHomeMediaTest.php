...
class MeHomeMediaTest extends TestCase
{
    ...
    public function test_upload_and_get_home_media_image(): void
    {
        ...
        $uploadedUrl = $upload->json('success.data.item.url');
        ...
        $this->assertIsString($uploadedUrl);
        $this->assertStringStartsWith('/storage/', $uploadedUrl);

        $get = $this->withHeaders(['X-Device-Id' => 'dev-home-media-001'])
            ->getJson('/api/me/media/home');

        $getUrl = $get->json('success.data.item.url');
        $get->assertStatus(200)
            ->assertJsonPath('success.data.item.type', 'image');
        $this->assertIsString($getUrl);
        $this->assertStringStartsWith('/storage/', $getUrl);
    }
