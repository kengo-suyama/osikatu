...
const DEVICE_ID = "device-e2e-legal-001";
...
test("terms page renders", async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="terms-page"]')).toBeVisible({ timeout: 15_000 });
});
...
test("privacy page renders", async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="privacy-page"]')).toBeVisible({ timeout: 15_000 });
});
