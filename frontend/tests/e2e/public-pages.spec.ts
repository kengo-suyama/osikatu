...
const DEVICE_ID = "device-e2e-public-001";
...
test("status page renders", async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="status-page"]')).toBeVisible({ timeout: 15_000 });
  await expect(page.locator('[data-testid="status-operational"]')).toBeVisible();
});
...
test("contact page renders", async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="contact-page"]')).toBeVisible({ timeout: 15_000 });
